...
For a biomarker with expected accuracy measure performance $A = A_{expected}$, this application uses Monte Carlo simulation to assess the performance of estimation procedures under nested case-control study design.
...
Estimates for all measures are calculated via semiparametric methods assuming a proportional hazards model using perturbation methods to account for the nested case-control design. An R package that implements the estimation procedures used in this application is available [**here**](http://mdbrown.github.io/survMarkerTwoPhase/). Please see the referenced papers for detailed information regarding estimation. 

***Note: Non-parametric estimation methods will be added shortly!***

### Resources

 - **Source Code** available upon request to mdbrown@fhcrc.org. 
